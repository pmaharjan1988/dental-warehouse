<template>
  <div>
    <div class="q-pa-xl">
      ...
      <div class=" text-center text-grey text-body1 mbb-30">
        Select from our large number of products
      </div>
      <div style="width:100%;height:400px;position: relative;">
        <!-- Using the slider component -->
        <slider ref="slider" :options="options">
          <!-- slideritem wrapped package with the components you need -->
          <slideritem v-for="(item,index) in slider_products" :key="index" :style="item.style" @tap="openProduct(item.id)" style="cursor:pointer">
            <div style="position:absolute; bottom:0px; width:95% !important;font-size: 0.8em !important; word-wrap: break-word;white-space: pre-line; padding:5px; color:#c3d9de;background-color: rgba(0, 0, 0, .3) !important;">{{item.name}}</div>
          </slideritem>
    ...
    initProducts: async function()
    {
      ...
      this.products_data.forEach(function(value, i)
      {
        sliderProducts[i] = {
          'id': value.id,
          'name': value.name,
          'style': 'display:flex;background-image:url(' + value.thumb_img + ');width:40%;margin-right:2%;',
        }
