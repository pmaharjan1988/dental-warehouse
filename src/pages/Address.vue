            ...
            <q-form @submit="onSubmit" class="q-gutter-md">
              <div class="row">
                <div class="col-md-6 col-sm-6 col-xs-12 q-pr-md q-pl-md">
                  <q-input filled dense v-model="address_form.name" label="Name *" lazy-rules :rules="[ val => val && val.length > 0 || 'Please Enter your Name name']" />
                </div>
                <div class="col-md-6 col-sm-6 col-xs-12  q-pr-md q-pl-md">
                  <q-input filled prefix="+61" dense type="number" v-model="address_form.mobile" label="Mobile Number *" lazy-rules :rules="[
              ...
              <q-input dense v-model="address_form.address" filled autogrow class=" q-pr-md q-pl-md" label="Address" lazy-rules :rules="[ val => val && val.length > 0 || 'Please Enter Address']" />
              <q-input dense v-model="address_form.landmark" filled autogrow class=" q-pr-md q-pl-md" label="Landmark" lazy-rules :rules="[ val => val && val.length > 0 || 'Please Enter Landmark']" />
              <q-input dense v-model="address_form.postalcode" filled class=" q-pr-md q-pl-md" label="Postalcode" lazy-rules :rules="[ val => val && val.length > 0 || 'Please Enter Postalcode']" />
              <div class=" flex justify-end">
                <q-btn label="Deliver to this Address" :loading="loading" type="submit" color="primary" />
              </div>
            </q-form>
          </q-card>
        </div>
  ...
  methods:
  {
    onSubmit: function()
    {
      if (this.address_data == null)
      {
        ...
        this.loading = true;
        firebaseStorage.collection('address').doc(enqId).set(
          {
      ...
      else
      {
        ...
        firebaseStorage.collection('address').doc(addId).update(
        {
          uid: this.address_form.uid,
          name: this.address_form.name,
          mobile: this.address_form.mobile,
